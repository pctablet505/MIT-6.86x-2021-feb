...
class NeuralNetwork():
    ...
    def train(self, x1, x2, y):
        ...
        d_output = np.vectorize(output_layer_activation_derivative)
        output_layer_error = (activated_output-y)  # TODO (1 by 1 matrix)

        hidden_layer_error = ((d_output(
            activated_output)@self.hidden_to_output_weights).T@output_layer_error)  # TODO (3 by 1 matrix)

        # TODO (3 by 1 matrix. element wise multiplication of two 3x1 matirices)
        bias_gradients = np.multiply(
            hidden_layer_error, d_relu(hidden_layer_weighted_input))

        hidden_to_output_weight_gradients = output_layer_error@hidden_layer_activation.T  # TODO
        input_to_hidden_weight_gradients = bias_gradients@input_values.T  # TODO
    ...
    def predict(self, x1, x2):
        ...
        hidden_layer_weighted_input = self.input_to_hidden_weights@input_values  # TODO
        hidden_layer_activation = np.vectorize(rectified_linear_unit)(
            hidden_layer_weighted_input)  # TODO
        output = self.hidden_to_output_weights@hidden_layer_activation  # TODO
        activated_output = np.vectorize(
            output_layer_activation)(output)  # TODO
