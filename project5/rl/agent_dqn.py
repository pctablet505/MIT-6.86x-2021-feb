...
def epsilon_greedy(state_vector, epsilon):
    ...
    q_value_next_action, q_value_next_obj = model(state_vector)
    action_index = torch.argmax(q_value_next_action)
    object_index = torch.argmax(q_value_next_obj)
    return (action_index, object_index)
...
def deep_q_learning(current_state_vector, action_index, object_index, reward,
    ...
    q_value = (q_value_cur_state[0][action_index] +
               q_value_cur_state[1][object_index])/2

    # TODO Your code here
    if terminal:
        q_value_max_next = 0
    else:
        q_value_max_next = maxq_next
    #loss = None
    # loss=1/2*(Y-q_value)**2
    # Y=(reward+gamma*q_value_max_next)
    loss = 1/2*(reward+GAMMA*q_value_max_next-q_value)**2
