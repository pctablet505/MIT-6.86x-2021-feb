...
def epsilon_greedy(state_vector, theta, epsilon):
    ...
    for ai in range(NUM_ACTIONS):
        for oi in range(NUM_OBJECTS):
            q_value = (theta@state_vector)[tuple2index(ai, oi)]
            if q_value > q_max:
                q_max = q_value
                action_index = ai
                object_index = oi
...
def linear_q_learning(theta, current_state_vector, action_index, object_index,
    ...
    if not terminal:
        next_q_value = max([(theta@next_state_vector)[tuple2index(ai, oi)]
                            for ai in range(NUM_ACTIONS) for oi in range(NUM_OBJECTS)])
    else:
        next_q_value = 0
    q_value = (
        theta@current_state_vector)[tuple2index(action_index, object_index)]
    theta[tuple2index(action_index, object_index)] += ALPHA * \
        (reward+GAMMA*next_q_value-q_value)*current_state_vector
