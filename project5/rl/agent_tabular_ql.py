...
def epsilon_greedy(state_1, state_2, q_func, epsilon):
    ...
    if np.random.random() <= epsilon:
        action_index, object_index = np.random.randint(
            NUM_ACTIONS), np.random.randint(NUM_OBJECTS)
    ...
    for ai in range(NUM_ACTIONS):
        for oi in range(NUM_OBJECTS):
            reward = q_func[state_1, state_2, ai, oi]
            if reward > max_reward:
                max_reward = reward
                action_index, object_index = ai, oi
    return (action_index, object_index)
...
def tabular_q_learning(q_func, current_state_1, current_state_2, action_index,
    ...
    """
    if not terminal:
        next_max_reward_possible = max([q_func[next_state_1, next_state_2, action_index_1, object_index_1]
                                        for action_index_1 in range(NUM_ACTIONS) for object_index_1 in
                                        range(NUM_OBJECTS)])
    else:
        next_max_reward_possible = 0
    q_func[current_state_1, current_state_2, action_index,
           object_index] = ((1 - ALPHA) * (q_func[current_state_1, current_state_2, action_index,
                                                  object_index]) +
                            ALPHA * (reward + GAMMA * next_max_reward_possible))
...
def run_episode(for_training):
    ...
    while not terminal:
        ...
        action_index, object_index = epsilon_greedy(
            state_1, state_2, q_func, epsilon)
        ...
        if for_training:
            ...
            tabular_q_learning(q_func, state_1, state_2,
                               action_index, object_index, reward, next_state_1, next_state_2, terminal)
